...
console.log(uploadResult);
    ...
    const result = await variationsCollection.insertOne(newDoc);
if (result.acknowledged) {
  console.log("✅ Data inserted successfully!");
  console.log("🆔 Inserted ID:", result.insertedId.toString());
  const variationId = result.insertedId.toString();
  UpdateProductById(variationId,productId)
}
...
async function UpdateProductById(variationId, productId) {
  ...
  const result = await productCollection.updateOne(
    ...
    {
      $push: { variationsId: variationObjectId }, // push variationId to array
    }
  );
}