...
import { getvariationsById } from "./variationsFunc";

// ✅ Zod validation schema


export function EditVariations() {
  const [client, setClient] = useState(false);
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState(null);
  const router = useRouter();
  const params = useParams();
  const variationdId = params.id;
  ...
  const onSubmit = async (data) => {
    try {
      ...
      formData.append("variationdId", variationdId);
      formData.append("imageUrl", data?.imageUrl);
      formData.append("publicId", data?.publicId);

      const response = await fetch("/api/variations/editvariations", {
        method: "POST",
        body: formData,
      });
      ...
      if (result.success) {
        setMessage("Success", "Variations Updated Successfully!");
        setLoading(false);
  ...
  const getVariationsHandle = async () => {
    const fetchedData = await getvariationsById(variationdId);
    setData(fetchedData);
