...
import { deleteVariationsById, getVariations } from "./variationsFunc";
...
export function ListVariations() {
  ...
  const getVariationsHandle = async (page, sort = 1, search) => {
    setLoading(true);
    console.log(`page111= ${page} sort = ${sort} search = ${search}`);
    const variations = await getVariations(page, sort, search, limit);
    console.log("variations Data", variations)
    setData(variations.data);
    setTotal(variations.total);
    setpageNme(page)
    setLoading(false);
  ...
  useEffect(() => {
    setclient(true);
    getVariationsHandle(1);

  }, [])

  const handelDeleteVariations = async (id, imageId) => {
   // console.log("image ID", imageId);
    ...
    const result = await deleteVariationsById(id, imageId);
    if (result) {
      setMessage("Success", "Variations Deleted Successfully!");
      await getVariationsHandle(1)
    }
  ...
  <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 border-b border-[#f9fafb] pb-4">
    <div className="w-full md:w-1/2">
      <div className="text-base md:text-xl font-semibold flex items-center uppercase gap-2">
        <List size={36} />
        <span>variations / Listing </span>
      </div>
    </div>
    <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
      <Search getVariationsHandle={getVariationsHandle} />
      <CatFilter getVariationsHandle={getVariationsHandle} />
    </div>
  </div>
  ...
  {data ? (
    <div className="w-full">
      ...
      <div className="hidden md:block overflow-x-auto rounded-xl shadow-sm bg-white border border-[#f9fafb]">
        <Table className="w-full text-sm text-gray-700">
          <TableHeader className="bg-gray-50 border-b border-[#f9fafb]">
            <TableRow>
              <TableHead className="py-3 px-4">Sr.No</TableHead>
              ...
              <TableHead className="py-3 px-4">pirce</TableHead>
              <TableHead className="py-3 px-4">stock</TableHead>
          ...
          <TableBody>
            {data.map((item, i) => (
              ...
              >
                ...
                <TableCell className="py-3 px-4">{item.price}</TableCell>
                <TableCell className="py-3 px-4">{item.stock}</TableCell>
                <TableCell className="py-3 px-4 line-clamp-2">
                  {item.description}
                </TableCell>
                <TableCell className="py-3 px-4 text-right">
                  <div className="flex justify-end gap-3">
                    <Link
                      href={`${domain}/editvariations/${item._id}`}
                    ...
                    <Delete
                      ...
                      onClick={() =>
                        handelDeleteVariations(item._id, item.publicId)
                      }
                    />
                  </div>
                </TableCell>
          ...
          >
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <Delete
                ...
                onClick={() => handelDeleteVariations(item._id, item.publicId)}
              />
            </div>
          </div>
        ))}
      ...
      <div className="border-t border-[#f9fafb] mt-6 pt-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-sm">
        ...
        <CatPagination
          total={totalPages}
          getVariationsHandle={getVariationsHandle}
          pageNum={pageNum}
        />
      </div>
    </div>
