...
import { deleteProductById, GetProducts } from "./productFunc";
...
export function ListProduct() {
  ...
  const getProductHandle = async (page, sort = 1, search) => {
    setLoading(true);
    console.log(`page= ${page} sort = ${sort} search = ${search}`);
    const Product = await GetProducts(page, sort, search, limit);
  ...
  const handleDeleteProduct = async (id, imageId) => {
    ...
    const result = await deleteProductById(id, imageId);
    if (result) {
      setMessage("Success", "Product Deleted Successfully!");
      await getProductHandle(1)
    }
      ...
      <div className="rounded-md flex flex-col text-[#252525] px-2 md:px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 border-b border-[#f9fafb] pb-4">
          ...
          <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
            <Search getProductHandle={getProductHandle} />
            <CatFilter getProductHandle={getProductHandle} />
          </div>
        </div>
              ...
              <Table className="w-full text-sm text-gray-700">
                <TableHeader className="bg-gray-50 border-b border-[#f9fafb]">
                  <TableRow>
                    ...
                    <TableHead className="py-3 px-4">Variations</TableHead>
                    ...
                    >
                      ...
                      <TableCell className="py-3 px-4 line-clamp-2">
                       ...
                       <TableCell className="py-3 px-4">
                        ...
                        {item.variaions.length>0 ? (
                          item.variaions.map((variation, index) => {
                            return <div key={index}>
                              <div>color: {variation.color}</div>
                              <div>size: {variation.size}</div>
                            </div>;
                          })
                        ):<div><Link href={`${domain}/addvariations`} className="">Add Variations</Link></div>}  
                      </TableCell>
                 	 <TableCell className="py-3 px-4">
                       

                        {item.categoryName && (
                          item.categoryName.map((cat, index) => {
                            return <div key={index}>{cat.name}</div>;
                          })
                        )}  
                      </TableCell>
                     
                      <TableCell className="py-3 px-4 text-right bg-red">
                        <div className="flex justify-end gap-3">
                          <Link
                            href={`${domain}/editproduct/${item._id}`}
                          ...
                          <Delete
                            ...
                            onClick={() =>
                              handleDeleteProduct(item._id, item.publicId)
                            }
                          />
                        </div>
                      </TableCell>
              ...
              />
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    ...
                    <Delete
                      ...
                      onClick={() => handleDeleteProduct(item._id, item.publicId)}
                    />
                  </div>
                </div>
              ))}
