...
export async function GetProducts(page=1,sort=1,search="",limit=4) {
...
console.log("Api Called Product", search)
  const queryString = new URLSearchParams({
    ...
    const response = await fetch(`/api/products/listProduct?${queryString}`);
...
export async function getProductById(id) {
  ...
  const queryString = new URLSearchParams({
    ...
    const response = await fetch(`${domain}/api/products/getProductById?${queryString}`);
...
export async function deleteProductById(id,imageID) {
  try {
    const queryString = new URLSearchParams({ id,imageID }).toString();

    const response = await fetch(`${domain}/api/products/deleteProduct?${queryString}`, {
