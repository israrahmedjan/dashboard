...
import { deletegalleryById, getGallery } from "./galleryOperations";
...
export function ListGallery() {
  ...
  const getGalleryHandle = async (page, sort = 1, search) => {
    setLoading(true);
    console.log(`page111= ${page} sort = ${sort} search = ${search}`);
    const variations = await getGallery(page, sort, search, limit);
  ...
  const handleDeleteGallery = async (id, imageId) => {
   // console.log("image ID", imageId);
    ...
    const result = await deletegalleryById(id, imageId);
    if (result) {
      setMessage("Success", "Gallery Deleted Successfully!");
      await getGalleryHandle(1)
    }
 ...
 <div className="rounded-md flex flex-col text-[#252525] px-2 md:px-6">
  ...
  <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 border-b border-[#f9fafb] pb-4">
    ...
    <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
      <Search getGalleryHandle={getGalleryHandle} />
      <CatFilter getGalleryHandle={getGalleryHandle} />
    </div>
  </div>
  ...
  {data ? (
    <div className="w-full">
              ...
              >
                ...
                <TableCell className="py-3 px-4 text-right">
                  <div className="flex justify-end gap-3">
                    <Link
                      href={`${domain}/editgallery/${item._id}`}
                    ...
                    <Delete
                      ...
                      onClick={() =>
                        handleDeleteGallery(item._id, item.publicId)
                      }
                    />
                  </div>
                </TableCell>
          ...
          >
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <Delete
                ...
                onClick={() => handleDeleteGallery(item._id, item.publicId)}
              />
            </div>
          </div>
        ))}
      ...
      <div className="border-t border-[#f9fafb] mt-6 pt-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-sm">
        ...
        <CatPagination
          total={totalPages}
          getGalleryHandle={getGalleryHandle}
          pageNum={pageNum}
        />
      </div>
    </div>
